...
public class CloudstackControllerImpl implements ResultListener {
	...
	public CloudstackControllerImpl(Properties properties) {
		super();
		this.client = new CloudStackAPI(properties.getProperty("apiURL"), properties.getProperty("secret"), properties.getProperty("apiKey"));
	}
	...
	public List<SupplySeries> getResourceAllocations(Date startDate, Date endDate, String ip) {
		...
		try {
			...
			List<ResourceAllocation> scheduledVMs = getResourceAllocationFromReply(events,startDate,endDate, VMCreateDestroy.instance(), EventState.SCHEDULED);
			List<ResourceAllocation> completedVMs = getResourceAllocationFromReply(events,startDate,endDate, VMCreateDestroy.instance(), EventState.COMPLETED);
			List<ResourceAllocation> completedLBVMs = getResourceAllocationFromReply(events,startDate,endDate, LBAddRemoveVM.instance(), EventState.COMPLETED);
	...
	private boolean deleteLoadBalancerRule(String id, boolean deleteHealthChecks) {
		if (deleteHealthChecks) {
			...
			if (healthCheckId != "") {
				System.out.println("Deleted healthCheck: " + healthCheckId + ": " + deleteLBHealthCheckPolicy(healthCheckId));
			}			
		}
	...
	public String createAutoScaleVMGroup(CloudSettings cs)
	{
		...
		CSLB csLB = createLoadBalancerRule(cs.getName(), cs.getIp(), cs.getAlgorithm().toString().toLowerCase(), cs.getPrivatePort(), cs.getPublicPort());
		createLBHealthCheckPolicy(csLB.id, cs.getHealthCheckPingPath(), cs.getHealthCheckInterval(), cs.getHealthCheckTimeout(), cs.getHealthCheckHealthyThreshold(), cs.getHealthCheckUnhealthyThreshold());
		...
		String scaleUpConditionId = createConditionUseCounterKeyword(scaleUp.getCounter(), scaleUp.getOperator(), scaleUp.getThreshold()).id;
		String scaleDownConditionId = createConditionUseCounterKeyword(scaleDown.getCounter(), scaleDown.getOperator(), scaleDown.getThreshold()).id;
		
		CSPolicy policyUp = createAutoScalePolicy(ScaleDirection.UP, scaleUpConditionId, scaleUp.getDuration(), scaleUp.getQuiettime());
		CSPolicy policyDown = createAutoScalePolicy(ScaleDirection.DOWN, scaleDownConditionId, scaleDown.getDuration(), scaleDown.getQuiettime());
		return createAutoScaleVMGroup(csLB.id, cs.getMinInstances(), cs.getMaxInstances(), policyUp.id, policyDown.id, vmProfileId, cs.getEvalInterval()).id;
	}
	...
	public boolean updateAutoScaleVMGroup(String ip, int minMembers, int maxMembers,  int interval) {
		...
		return updateAutoScaleVMGroup(autoScaleGroupForIp.id, minMembers, maxMembers, autoScaleGroupForIp.scaleUpPolicies, autoScaleGroupForIp.scaleDownPolicies, interval);
	}
	
	public boolean updateAutoScaleVMGroup(String ip, int minMembers, int maxMembers) {
		CSAutoScaleVMGroup autoScaleGroupForIp = getAutoScaleGroupForIp(ip);
		return updateAutoScaleVMGroup(autoScaleGroupForIp.id, minMembers, maxMembers, autoScaleGroupForIp.scaleUpPolicies, autoScaleGroupForIp.scaleDownPolicies, autoScaleGroupForIp.interval);
	}
	...
	private boolean deleteAutoScaleVMGroup(String id, boolean recursive) {
		...
		if (recursive)
		{
			System.out.println("Deleted profile: " + group.vmProfileId + ": " + deleteAutoScaleVmProfile(group.vmProfileId));
			System.out.println("Deleted policyUp: " + group.scaleUpPolicies + ": " + deleteAutoScalePolicy(group.scaleUpPolicies, true));
			System.out.println("Deleted policyDown: " + group.scaleDownPolicies + ": " + deleteAutoScalePolicy(group.scaleDownPolicies, true));
			// TODO: lbRuleId is null... don't know why, Cloudstack bug?
			// System.out.println("Deleted lb: " + group.lbRuleId + ": " + deleteLoadBalancerRule(group.lbRuleId));
		}
		return success;
	}
	
	private String createLBHealthCheckPolicy(String lbruleid, String pingpath, int intervaltime, int timeout, int healthythreshold, int unhealthythreshold) {
		String id = createLBHealthCheckPolicy(lbruleid, pingpath, intervaltime, timeout, healthythreshold, unhealthythreshold, this); 
		waitForAsyncResult();
		return id;
	}
	...
	private CSAutoScaleVMGroup createAutoScaleVMGroup(String lbRuleId, int minMembers, int maxMembers, String scaleUpPolicyIds,String scaleDownPolicyIds, String vmProfileId, int interval)
	{
		String id = createAutoScaleVMGroup(lbRuleId, minMembers, maxMembers, scaleUpPolicyIds, scaleDownPolicyIds, vmProfileId, interval, this);
		waitForAsyncResult();
		return getAutoScaleGroupById(id);
	}
	
	private boolean updateAutoScaleVMGroup(String vmGroupId, int minMembers, int maxMembers, String scaleUpPolicyIds, String scaleDownPolicyIds, int interval) {
		updateAutoScaleVMGroup(vmGroupId, minMembers, maxMembers, scaleUpPolicyIds, scaleDownPolicyIds, interval, this);
		return waitForAsyncResult();
	}
	
	public CSCondtion createConditionUseCounterKeyword(String counterKeyword, String relationalOperator, int threshold) {
	...
	public String setScaleUpPolicyDuration(String ip, ScaleDirection direction, int durationInSeconds, ResultListener listener) 
	{
	...
	private String createConditionUseCounterKeyword(String counterKeyword, String relationalOperator, int threshold, ResultListener listener) {
		HashMap<String,String> options = new HashMap<>();
		options.put("keyword", counterKeyword);
		String counterId = getCounterByOptions(options);
		return createConditionUseCounterId(counterId, relationalOperator, threshold,
				listener);
	}

	private String createConditionUseCounterId(String counterId, String relationalOperator,
			int threshold, ResultListener listener) {
		String resourceId = "";
		HashMap<String,String> options = new HashMap<>();
		try {
	...
	private String createAutoScaleVMGroup(String lbRuleId, int minMembers, int maxMembers, String scaleUpPolicyIds, String scaleDownPolicyIds, String vmProfileId, int interval, ResultListener listener) {
		String resourceId = "";
		HashMap<String,String> options = new HashMap<>();
		options.put("interval", Integer.toString(interval));
		try {
			Document reply = client.createAutoScaleVmGroup(lbRuleId, minMembers, maxMembers, scaleDownPolicyIds, scaleUpPolicyIds, vmProfileId, options);
			registerListenerAsyncJobResult(listener, reply);
			resourceId = getResourceId(reply);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private String updateAutoScaleVMGroup(String vmGroupId, int minMembers, int maxMembers, String scaleUpPolicyIds, String scaleDownPolicyIds, int interval, ResultListener listener) {
		String resourceId = "";
		HashMap<String,String> options = new HashMap<>();
	...
	private String createAutoScaleVMProfile(String serviceOffering, String templateName, String ip, ResultListener listener) {
		CSIp csIP = getIPbyIP(ip);
		String resourceId = "";
		HashMap<String,String> options = new HashMap<>();
	...
	private String createAutoScalePolicy(ScaleDirection direction, String conditionIds, int duration, int quiettime, ResultListener listener) {
		String resourceId = "";
		HashMap<String,String> options = new HashMap<>();
	...
	private String createLBHealthCheckPolicy(String lbruleid, String pingpath, int intervaltime, int timeout, int healthythreshold, int unhealthythreshold, ResultListener listener) {
		String resourceId = "";
		HashMap<String,String> options = new HashMap<>();
	...
	private String createLoadBalancerRule(String name, String ip, String algorithm, int privatePort, int publicPort, ResultListener listener) {
		CSIp csIP = getIPbyIP(ip);
		String resourceId = "";
		HashMap<String,String> options = new HashMap<>();
		...
		try {
			Document reply = client.createLoadBalancerRule(algorithm, name, Integer.toString(privatePort), Integer.toString(publicPort), options);
			registerListenerAsyncJobResult(listener, reply);
			resourceId = getResourceId(reply);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void setConditionThresholdForId(String policyId, int threshold, ResultListener listener) {
		...
		CSCondtion newCondition = createConditionUseCounterId(condition.counterId, condition.relationalOperator, threshold);
	...
	private String registerListenerAsyncJobResult(ResultListener listener,
			Document reply) throws XPathExpressionException {
		String jobId;
		XPathFactory factory = XPathFactory.newInstance();
		XPath xpath = factory.newXPath();
	...
	private String getScalePolicyForGroupId(String groupId, ScaleDirection direction) {
		...
		try {
			...
			XPathFactory factory = XPathFactory.newInstance();
		    XPath xpath = factory.newXPath();
		    XPathExpression xp_property = xpath.compile("/listautoscalevmgroupsresponse/autoscalevmgroup/scale" + direction + "policies/id/text()");
	...
	private static List<ResourceAllocation> getResourceAllocationFromReply(Document reply, Date start, Date end, EventType type, EventState status) {
		List<ResourceAllocation> allocations = new LinkedList<ResourceAllocation>();

		/* Cloudstack returns second precision only
		 * -> cut precision of start and end date to second to simplify date comparisons
		 */
