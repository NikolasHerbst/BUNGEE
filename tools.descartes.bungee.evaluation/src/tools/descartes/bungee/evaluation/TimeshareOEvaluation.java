...
public class TimeshareOEvaluation extends CloudStackExperiment {
	
	private int[] durations;

	public TimeshareOEvaluation(CloudstackControllerImpl cloudController) {
		super(cloudController);
		expFolder = new File(FileUtility.FILE_LOCATION, "evaluation/timeshareO");
		measurementConfigFile = new File(expFolder,"measurement.prop");
		evaluateExistingMeasurement = true;
	}

	public void before() {
		measurementFolder = new File(expFolder,"2014.05.05_20.15.24");
	...
	public void run() {
		...
		{
			System.out.println("Start Experiment: TimeshareO Evaluation");
			for (int duration : durations)
			{
				...
				System.out.println("Run for scale down duration: " + duration);
